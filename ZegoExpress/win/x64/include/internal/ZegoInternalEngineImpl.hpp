...
class ZegoExpressEngineImp : public IZegoExpressEngine {
  public:
	...
	_config.adaptive_template_id_count = (unsigned int)config.adaptiveTemplateIDList.size();
	    ...
	    for (unsigned int i = 0; i < _config.adaptive_template_id_count; ++i) {
    ...
    void setAudioMixMode(ZegoAudioMixMode mode,
        ...
        oInternalOriginBridge->setAudioMixMode(static_cast<zego_audio_mix_mode>(mode),
                                               stream_list_.data(), (int)stream_list_.size());
    }
	...
	message_.content_length = (unsigned int)message.content.length();
	...
	oInternalOriginBridge->customVideoCaptureRegionOfInterest(c_rects.data(), (unsigned int)c_rects.size(),
                                                                  zego_publish_channel(channel));
    }
